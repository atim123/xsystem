...
class ExclusiveGatewayComposer extends FlowElementComposer {

    @Override
    public Element composer(Element root, Object src) {
        ExclusiveGateway exclusiveGateway = (ExclusiveGateway) src;
        Element ret = makeBaseElement(root, "exclusiveGateway", exclusiveGateway);
        String gatewayDirection = exclusiveGateway.getGatewayDirection().toString();
