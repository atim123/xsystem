 ...
 */
public class EndEventExecution extends Execution {

    public EndEventExecution(ExecutionService owner) {
        super(owner);
    }
    ...
    public void execute(Token token) {
        ...
        EndEvent endEvent = (EndEvent) ref.resolvedReference();

        String procId = endEvent.getProcess().getId();

        long termited = endEvent.getEventDefinitions().stream()
                .filter(predicate -> predicate instanceof TerminateEventDefinition)
                .count();
        if (termited == 0) {
            owner.destroyToken(token);
        } else {
            List<Token> activeToken = owner.getActiveToken(procId);
            activeToken.forEach(action -> {
